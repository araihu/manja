...
claim. `Pack` may write deterministic caller-output archives, SBOM bytes, and
notice-manifest bytes only after both authority receipts and all byte/inventory
checks pass; blocked requests leave the caller output untouched.
...
a complete CycloneDX-JSON or SPDX-JSON SBOM receipt, a deterministic
`Manja-Notice-Manifest-JSON` receipt, and a recursive regular file inventory
with sizes and digests. The notice manifest contains only caller-supplied
shipped dependency facts; it is an inventory aid, not a license grant or a
replacement for `THIRD_PARTY_NOTICES.md`. Artifact dependency names must
resolve to exactly one dependency evidence record; build-only and test-only
records cannot enter a produced artifact.
...
an inventory. Existing SBOM and notice-manifest bytes are checked against
deterministic generated bytes before staging. Build-only, test-only, parser,
and compiler dependencies remain excluded unless final artifact bytes prove
otherwise.

## Receipt shapes

Every artifact receipt uses common `name`, `kind`, `source`, `digest`,
`inspection`, `sbom`, `noticeManifest`, `dependencies`, and complete `files`
fields. Kind-specific source contracts are fail-closed:

| Kind | Required source/inspection evidence |
| --- | --- |
| `source-archive` | Complete recursive source-root inventory; source-only files are inspected and never silently omitted. |
| `binary-archive` | Complete fresh-root inventory of final archive, including legal files, SBOM, and notice manifest. |
| `oci` | Exact `ghcr.io/araihu/manja@sha256:<64 lowercase hex>` reference, complete platform coverage, and one digest-bound receipt per published OS/architecture manifest. Tags and caller directories are not OCI receipts. |
| `site` | Complete fresh-root inventory of supplied site artifact, including legal files, SBOM, notice manifest, and embedded static bytes. A site test pass without final artifact bytes is not a receipt. |

`Pack` stages every requested archive in a private directory and publishes the
output directory only after all artifacts, legal bytes, SBOMs, notice
manifests, and final inspections pass. Existing output directories are never
overwritten or mixed with a partial result.
...
platform, and artifact-dependency arrays. Generated SBOM and notice-manifest
bytes contain no timestamps, UUIDs, host paths, or environment-dependent
fields. `check` emits a raw deterministic receipt and exits `0` for `PASS`,
`1` for `BLOCKED`, and `2` for malformed
...
runtime sources, unsafe paths and policy placements, non-regular files,
missing roots, recursive inventory drift, incomplete or drifted SBOM and notice
manifest bytes, archive extraction, atomic multi-artifact staging, digest-bound
OCI references and coverage, deterministic bytes, and strict JSON decoding.
Test fixtures are
