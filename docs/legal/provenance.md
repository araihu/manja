...
The bounded external-source receipt
[`external-source-provenance.md`](external-source-provenance.md) records the
immutable Arai Hû Assets `v0.2.1` release and `origin/main` identities, the
four manifest-selected asset paths and installed byte matches, Assets
`LICENSE`/`NOTICE`, and the pinned Goshtoso `v0.3.0` module `LICENSE`. Its
tests recompute size, Git blob SHA-1, and SHA-256 offline. This is mechanical
evidence only: first-party authority, final-artifact attribution and notice
...
That command block is historical evidence for `AUDITED_BASE`, which predates
the separate developer-tool module. In the current tree, the Muamba equivalent
is `go tool -modfile=tools/go.mod muamba verify --strict`.
...
| Arai Hû Assets `v0.2.1` static inputs | [`external-source-provenance.json`](external-source-provenance.json) binds `araihu/assets` release commit `fdfb1c2aad8fa61779e7b8c6f208e52a6cf825ce`, tree `e89dce3f6bbb129bf3dfd3f04d874cdb220bb4a0`, archive SHA-256 `818a32246c040871c8f28bb085269b6b9f21c579b18dc4c3c1f20d70716eaf70`, and origin/main commit/tree. It covers all four `araihu-assets.json` mappings and compares every installed copy by size, Git blob, and SHA-256. The same receipt identifies upstream `LICENSE` as Apache-2.0 and `NOTICE` as a 633-byte file. | Mechanical source-byte identity is resolved. Attribution/notice placement in final artifacts, first-party authority, and Arai Hû/Manja mark and trademark clearance remain **BLOCKED**. |
| Goshtoso `v0.3.0` runtime module | [`external-source-provenance.json`](external-source-provenance.json) binds `github.com/araihu/goshtoso` `v0.3.0` to tag commit `2c4e86ab838868335e99f485bbab8caa88b54c8b`, tag tree `37ac981351457c03d60e907dbb3ad310c0fdeb2d`, and origin/main commit/tree. It compares the module `LICENSE` (MIT) by size, Git blob, and SHA-256. Runtime/browser redistribution and its transitive assets still require final artifact inventory and the OC-02 notice/SBOM gate. | Mechanical module/license identity is resolved. Runtime dependency notice placement, first-party authority, redistribution, and any trademark/endorsement clearance remain **BLOCKED**. |
