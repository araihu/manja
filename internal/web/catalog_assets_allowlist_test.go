...
func TestCatalogAssetsAllowOnlyCanonicalPublicAssetRequests(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "browser test asset", method: http.MethodGet, target: "/manja-assets/catalog-search.test.mjs", want: http.StatusNotFound},
		{name: "local docs test asset", method: http.MethodGet, target: "/manja-assets/local-docs/_static.test.mjs", want: http.StatusNotFound},
		{name: "Go test source", method: http.MethodGet, target: "/manja-assets/local_docs_browser_test.go", want: http.StatusNotFound},
