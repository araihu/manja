...
func (handler *CatalogHandler) catalogPageDataWithSidebarQuery(
	...
	if selectedChild != "" {
		...
		if detail.Operation != nil {
			operation, parameterNodes, requestBodyNodes, responseMediaNodes, responseDetailNodes, schemaTreeNodes, err := handler.catalogOperationView(ctx, snapshot, document, *detail.Operation)
			if err != nil {
				return templates.CatalogPageData{}, err
			}
			...
			operationResponseMedia, err := localrender.PrepareOperationResponseMedia(detail, *operation, responseMediaNodes, documentHref, data.SchemaLinks)
			if err != nil {
				return templates.CatalogPageData{}, err
			}
			data.OperationResponseMedia = &operationResponseMedia
			operationResponseDetails, err := localrender.PrepareOperationResponseDetails(detail, *operation, responseDetailNodes, documentHref, data.SchemaLinks)
			if err != nil {
				return templates.CatalogPageData{}, err
			}
			data.OperationResponseDetails = &operationResponseDetails
			operationExamples, err := localrender.PrepareOperationExamples(detail, *operation, responseMediaNodes)
			if err != nil {
				return templates.CatalogPageData{}, err
			}
			data.OperationExamples = &operationExamples
			operationSchemaTrees, err := localrender.PrepareOperationSchemaTrees(detail, *operation, schemaTreeNodes, documentHref, data.SchemaLinks)
			if err != nil {
				return templates.CatalogPageData{}, err
			}
			...
			if len(detail.Operation.Security) > 0 || len(detail.Operation.Parameters) > 0 || detail.Operation.HasRequestBody || len(detail.Operation.Responses) > 0 {
				operationDetailSections, err := localrender.PrepareOperationDetailSections(detail, *operation, data.OperationRequestSection, data.OperationResponses)
				if err != nil {
					return templates.CatalogPageData{}, err
				}
				data.OperationDetailSections = &operationDetailSections
			}
		...
		if detail.Schema != nil {
			...
			if data.SchemaView != nil {
				schemaHeader, err := localrender.PrepareSchemaDetailHeader(detail, *data.SchemaView, document, data.SchemaNode)
				if err != nil {
					return templates.CatalogPageData{}, err
				}
				...
				schemaDetail, err := localrender.PrepareSchemaDetail(detail, *data.SchemaView, document, &schemaHeader, &schemaBody)
				if err != nil {
					return templates.CatalogPageData{}, err
				}
				data.SchemaDetail = &schemaDetail
			}
